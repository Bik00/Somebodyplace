	...
	function enter(event, keyword) {
		...
		if(x==13) {
			...
			if(keyword=="도움말") {
				var result = "<div class='chat system'>사용할 수 있는 커멘드는 다음과 같습니다.<ul><li>도움말 - 사용할 수 있는 커멘드 기능들을 불러옵니다.</li><li>주문 - 상대방의 물품을 구매할 수 있습니다.<br><h6>예시 : 주문 청바지 등</li><li>상대상품 - 상대방의 플레이스에 등록된 상품들을 불러옵니다.</li><li>상대주소 - 상대 플레이스의 위치를 불러옵니다. (클릭 시 길 안내 가능)<li>내상품 - 내 플레이스에 등록된 상품들을 불러옵니다.</li><li>키워드 - 상대방이 입력한 키워드를 알 수 있습니다.</ul><h6>작성자 : 시스템 <br>작성 시간 : "+string+"</h6> </div>";
				$(result).appendTo(".chats");
			}
			if(keyword=="내상품") {
